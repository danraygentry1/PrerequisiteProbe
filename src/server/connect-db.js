...
export async function getPTSchoolCourseInfo(pool){
    ...
    let ptSchoolCourseColumnsArray = [];
    let ptSchoolCourseRowsArray = [];
    ...
    await (async () => {
        ...
        try {
            ...
            let i = 0
            resPT.rows.map(function (rowkey){
                ptSchoolCourseColumnsArray.map(function (columnkey){
...
export async function getPTUser(pool, username)
{
    ...
    try {
        ...
        let i = 0
        resUser.fields.map(function (key){
            ptUserColumns.push({key: key.name})
        })
        resUser.rows.map(function (rowkey){
            ptUserColumns.map(function (columnkey){
